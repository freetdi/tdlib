...
void test_parallel_instance_dynamic(std::string fin, std::string fout, std::string name, std::vector<bool> &job_finished, std::vector<bool> &threads_finished, unsigned int idx1, unsigned int idx2){
...
*/
    ...
    std::ofstream texresults("./Results/9_exact_dynamic.tex", std::ios::app);
...
void test_exact_dynamic(){
    ...
    package.td_dest = "./Decompositions/exact_dynamic/"; packages.push_back(package);
    ...
    for(int i = 0; i < packages.size(); i++){
        ...
        while(std::find(tasks_finished.begin(), tasks_finished.end(), false) != tasks_finished.end()){
            ...
            if(it == threads_finished.end() || all_scheduled){
                ...
                boost::thread t(test_parallel_instance_dynamic, *it_in, *it_out, *it_name, boost::ref(tasks_finished), boost::ref(threads_finished), job, pos);
                it_in++;
                it_out++;
                it_name++;
                job++;
    ...
    texresults.open("./Results/9_exact_dynamic.tex", std::ios::app);
