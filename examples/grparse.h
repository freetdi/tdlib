...
public:
	...
	std::pair<iterator, iterator> edges(){
...
namespace boost{
	size_t num_vertices(PARSE const& p){
		return p.num_vertices();
	}
	size_t num_edges(PARSE const& p){
		return p.num_edges();
	}
	std::pair<PARSE::iterator, PARSE::iterator> edges(PARSE& p){
		return p.edges();
	}
	template<>
	struct graph_traits<PARSE>{
		typedef size_t vertex_descriptor;
		typedef PARSE::iterator edge_iterator;
	};
} // boost
